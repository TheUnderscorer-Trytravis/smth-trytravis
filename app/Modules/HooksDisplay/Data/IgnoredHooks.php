...
final class IgnoredHooks
{
     ...
     * @return bool
     */
    public static function isIgnored( string $tag ): bool
    {
        $hooks = self::getHooks();

        if ( in_array( $tag, $hooks ) ) {
            return true;
        }
        ...
        foreach ( $hooks as $hook ) {
            // Search for variable hooks
            if ( ! Str::contains( $hook, '{$' ) ) {
                continue;
            }

            $formattedHook = preg_replace( $regex, '', $hook );
            $hookValue     = str_replace( $formattedHook, '', $tag );
            $hookWithValue = preg_replace( $regex, $hookValue, $hook );

            if ( $hookWithValue === $tag ) {
                return true;
            }
    ...
    {
        return apply_filters( 'smth.ignoredHooks', self::$ignoredHooks );
    }
}
